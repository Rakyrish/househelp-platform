...
const UserTable = ({ users }: { users: User[] }) => {
  ...
  const [roleFilter, setRoleFilter] = useState<"all" | "worker" | "employer">(
    "all",
  );
  ...
  const confirmPermanentDelete = (onConfirm: () => void) => {
    let typedValue = "";
    ...
    Modal.confirm({
      ...
      onOk: async () => {
        if (typedValue !== "DELETE") {
          message.error("Confirmation text mismatch");
        ...
        if (open) {
          setTimeout(() => {
            ...
            const observer = new MutationObserver(() => {
              okBtn.disabled = typedValue !== "DELETE";
            });
  ...
  return (
    <div className="w-full space-y-6">
                      ...
                      {!showTrash ? (
                        <>
                          ...
                          <button
                            ...
                            className="p-2 bg-white/5 text-amber-500 hover:bg-amber-500 hover:text-white rounded-lg"
                          >
                            <KeyOutlined />
                          </button>
                          <button
                          ...
                          <button
                            onClick={() => navigate(`/admin/verify/${user.id}`)}
                            className="text-[10px] font-bold uppercase px-3 py-1.5 bg-cyan-500/10 text-cyan-400 rounded-lg hover:bg-cyan-500 hover:text-black"
                          >
                            Manage
                          </button>
                          <button
                            onClick={() =>
                              handleAction("trash_user", user.id, "Trash?")
                            }
                            className="p-2 bg-white/5 text-slate-500 hover:text-red-400 rounded-lg"
                          >
                            <DeleteOutlined />
                          </button>
                        </>
                        ...
                        <>
                          <button
                            onClick={() =>
                              handleAction("restore_user", user.id, "Restore?")
                            }
      ...
      <Modal
        ...
        okButtonProps={{
          className: "bg-cyan-500 text-black border-none font-bold",
        }}
      ...
      >
        <div className="py-4">
          <Alert
            className="mb-6 bg-amber-500/10 border-amber-500/10 text-amber-200"
      ...
      <Modal
        ...
        centered
        styles={{
          body: { backgroundColor: "#0f172a", padding: 0, overflow: "hidden" },
        }}
      >
        {previewUser && (
          <div className="text-slate-200">
              ...
              <div className="flex justify-between items-start">
                <div className="flex items-center gap-5">
                  ...
                  <div>
                    ...
                    <Badge
                      status={previewUser.is_verified ? "success" : "warning"}
                      text={
                ...
                <Button
                  type="primary"
                  onClick={() => navigate(`/admin/verify/${previewUser.id}`)}
                >
                  Audit Full Docs
                </Button>
              </div>
            ...
            <div className="p-8">
              <Descriptions column={2} layout="vertical">
                ...
                >
                  <p className="m-0 text-sm capitalize">
                    <EnvironmentOutlined className="mr-2" />{" "}
                    {previewUser.location || "Unknown"}
                  </p>
                </Descriptions.Item>
                ...
                >
                  <span className="text-cyan-400 font-bold">
                    {previewUser.worker_type || previewUser.id_number || "N/A"}
                  </span>
                </Descriptions.Item>
              </Descriptions>
            </div>
