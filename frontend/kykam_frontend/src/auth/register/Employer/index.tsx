...
const RegisterEmployer = () => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (!formData.accepted_terms) return message.error("You must agree to the Terms and Conditions to continue.");
          ...
          <div className="bg-white rounded-3xl shadow-sm border border-slate-100 overflow-hidden">
            ...
            <div className="bg-[#f3a82f] px-8 py-10 text-white relative">
              <div className="relative z-10">
                ...
                <h2 className="text-3xl font-extrabold tracking-tight">Hire with Kykam</h2>
                <p className="mt-2 text-orange-50 opacity-90 font-medium">Find reliable, vetted domestic help for your home.</p>
              </div>
              <Home className="absolute right-8 top-1/2 -translate-y-1/2 opacity-10" size={120} />
            </div>
            ...
            <form onSubmit={handleSubmit} className="p-8 space-y-10">
              ...
              <section className="bg-slate-50 -mx-8 px-8 py-8 border-y border-slate-100">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-5">
                  <FormInput label="Home Location*" icon={MapPin} name="location" type="text" value={formData.location} onChange={handleChange} required placeholder="e.g. Kilimani, Nairobi" />

                  <div className="flex flex-col space-y-1.5">
                    <label className="text-xs font-bold text-slate-500 uppercase tracking-wider ml-1">Type of Help Needed*</label>
                    <select name="worker_type" value={formData.worker_type} onChange={handleChange} className="w-full px-4 py-2.5 bg-white border border-slate-200 rounded-xl focus:ring-2 focus:ring-[#f3a82f]/20 outline-none cursor-pointer">
                  ...
                  <div className="grid grid-cols-2 gap-4">
                    <FormInput label="Family Size*" icon={Users} name="family_size" type="text" inputMode="numeric" value={formData.family_size} onChange={handleNumericChange} required placeholder="e.g. 4" />
                    <FormInput label="Salary (Ksh)*" icon={Banknote} name="salary" type="text" inputMode="numeric" value={formData.salary} onChange={handleNumericChange} required placeholder="15000" />
                  </div>

                  <div className="flex flex-col space-y-1.5">
                    <label className="text-xs font-bold text-slate-500 uppercase tracking-wider ml-1">Accommodation Type*</label>
                    <select name="accommodation" value={formData.accommodation} onChange={handleChange} className="w-full px-4 py-2.5 bg-white border border-slate-200 rounded-xl focus:ring-2 focus:ring-[#f3a82f]/20 outline-none cursor-pointer">
                      <option value="live_out">Live Out (Commuter)</option>
                      <option value="live_in">Live In (Stay over)</option>
                    </select>
                  </div>

                  <FormInput label="Target Start Date" icon={Calendar} name="start_date" type="date" value={formData.start_date} onChange={handleChange} />

                  <div className="md:col-span-2 flex flex-col space-y-1.5">
                    <label className="text-xs font-bold text-slate-500 uppercase tracking-wider ml-1">Specific Requirements / Notes</label>
                    <textarea
                      name="requirements"
                      rows={3}
              ...
              <div className="flex items-start gap-3 p-4 bg-slate-50 border border-slate-200 rounded-xl">
                ...
                <label htmlFor="accepted_terms" className="text-sm text-slate-600 leading-relaxed cursor-pointer select-none">
                  I agree to the <button type="button" onClick={() => setIsTermsModalOpen(true)} className="text-[#f3a82f] font-bold hover:underline bg-transparent border-none p-0 cursor-pointer">Terms and Conditions</button> and consent to the collection and processing of my personal data for verification purposes.
                </label>
              </div>
            ...
            <div className="bg-slate-50 p-6 text-center border-t border-slate-100">
              <p className="text-slate-500 font-medium">
                ...
                <button onClick={() => navigate('/login/employer')} className="text-[#f3a82f] font-bold hover:underline cursor-pointer">
                  Log in here
                </button>
              </p>
            </div>
